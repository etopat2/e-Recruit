...
<template>
  <section class="page-heading"><p class="eyebrow">Support and accountability</p><h1>Helpdesk</h1><p>Ask for assistance without sharing passwords or authenticator codes. Appeals tied to a decision should be submitted from the application status record.</p></section><div v-if="notice" class="alert success page-alert">{{ notice }}</div><div v-if="error" class="alert error page-alert">{{ error }}</div><section class="content-section compact-top"><div class="section-heading"><div><p class="eyebrow">My requests</p><h2>{{ tickets.length }} support request(s)</h2></div><button type="button" class="button primary" @click="createDialogOpen = true">Open support request</button></div><article v-for="ticket in tickets" :key="ticket.id" class="ticket-card"><div><strong>{{ ticket.subject }}</strong><StatusBadge :status="ticket.status" /></div><p>{{ ticket.category }} · {{ new Date(ticket.created_at).toLocaleString() }}</p><code>{{ ticket.id }}</code></article><div v-if="tickets.length === 0" class="empty-state"><p>No support requests.</p></div></section>
  <Dialog :open="createDialogOpen" title="Open a support request" description="Never include passwords or authenticator codes in a support request." :close-on-backdrop="false" mobile-sheet @close="createDialogOpen = false"><form @submit.prevent="create"><label>Campaign ID<input v-model="form.recruitment_campaign_id" required data-dialog-initial-focus /></label><label>Application ID (if applicable)<input v-model="form.application_id" /></label><label>Category<select v-model="form.category"><option>application</option><option>document</option><option>access</option><option>interview</option><option>appeal</option><option>other</option></select></label><label>Subject<input v-model="form.subject" required /></label><label>Description<textarea v-model="form.description" rows="7" required /></label><button class="button primary full" :disabled="busy">{{ busy ? 'Submitting…' : 'Submit request' }}</button></form></Dialog>
</template>
