...
async function queueEvent() {
  ...
  try {
    ...
    await offlineDb.events.add({
      ...
      action_type: definition.value.action, payload_schema_version: 1, sealedPayload: await sealValue(eventPayload()),
...
async function sync() {
  ...
  try {
    ...
    const eventsToSend = await Promise.all(pending.map(async (event) => ({
      ...
      payload_schema_version: event.payload_schema_version, payload: event.payload || (event.sealedPayload ? await openValue<Record<string, unknown>>(event.sealedPayload) : {}),
...
<template>
  ...
  <section class="offline-grid">
    <article class="form-panel"><h2>1. Bind and provision</h2><label>Device UUID<input v-model="deviceId" readonly /></label><button v-if="!deviceRecordId" class="button secondary" @click="register">Register this device</button><template v-else-if="!packageId"><label>Pack purpose<select v-model="provision.packType"><option v-for="(item, key) in definitions" :key="key" :value="key">{{ item.label }}</option></select></label><label>Scoped entity IDs<textarea v-model="provision.entityIds" placeholder="One or more IDs, comma-separated" /></label><label>Scope (JSON)<textarea v-model="provision.scopeJson" rows="3" /></label><button class="button secondary" :disabled="!online" @click="issue">Issue scoped pack</button></template><template v-else><p class="mono-note">Pack {{ packageId }}</p><p><StatusBadge :status="packPayload?.package.status || 'active'" /> {{ definition.label }} · {{ packPayload?.server_records.length || 0 }} record(s)</p><p>Expires {{ new Date(packPayload?.package.expires_at || '').toLocaleString() }}</p><div class="button-row"><button class="button secondary compact" :disabled="!online" @click="pullChanges">Pull changes</button><button class="button danger compact" :disabled="!online" @click="revokePack">Revoke pack</button></div></template></article>
    <article class="form-panel"><h2>2. Capture {{ definition.label.toLowerCase() }}</h2><label>Pack record<select v-model="capture.entityId" :disabled="!packageId"><option v-for="record in packPayload?.server_records" :key="record.entity_id" :value="record.entity_id">{{ record.payload.application_reference || record.payload.panel_code || record.entity_id }} · v{{ record.server_version }}</option></select></label>
      ...
      <div v-if="selectedRecord" class="notice"><strong>Server snapshot v{{ selectedRecord.server_version }}</strong><pre>{{ JSON.stringify(selectedRecord.payload, null, 2) }}</pre></div><button class="button primary" :disabled="!packageId" @click="queueEvent">Queue encrypted event</button></article>
  </section>
  <section class="content-section compact-top"><div class="section-heading"><div><h2>Event outbox</h2><p>{{ activeEvents.filter((event) => event.state === 'pending').length }} awaiting acknowledgement · {{ conflicts.filter((item) => item.status === 'open').length }} open conflict(s)</p></div><div><label class="checkbox compact"><input v-model="completePack" type="checkbox" /> <span>Final sync: reconcile and purge</span></label><button class="button primary" :disabled="syncBusy || !online || !activeEvents.some((event) => event.state === 'pending')" @click="sync">{{ syncBusy ? 'Synchronising…' : 'Synchronise now' }}</button></div></div><div class="table-wrap"><table><thead><tr><th>Sequence</th><th>Event UUID</th><th>Action</th><th>Entity</th><th>State</th></tr></thead><tbody><tr v-for="event in activeEvents" :key="event.id"><td>{{ event.local_sequence }}</td><td><code>{{ event.id }}</code></td><td>{{ event.action_type }}</td><td>{{ event.entity_id }}</td><td><StatusBadge :status="event.state" /><small v-if="event.error">{{ event.error }}</small></td></tr><tr v-if="!activeEvents.length"><td colspan="5">No local events for this pack.</td></tr></tbody></table></div><div v-if="conflicts.length" class="notice"><strong>Supervisor resolution required</strong><p v-for="item in conflicts" :key="item.id">{{ item.field_key }} on {{ item.entity_id }}: local {{ JSON.stringify(item.local_value) }}, server {{ JSON.stringify(item.server_value) }}</p></div></section>
</template>
