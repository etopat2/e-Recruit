...
const hold = reactive({ entity_type: 'notifications', entity_id: '', entity_label: '', reason: '' })
...
async function placeHold() { await act('/governance/legal-holds', { entity_type: hold.entity_type, entity_id: hold.entity_id, reason: hold.reason }, 'Legal hold placed; matching purge candidates are excluded.') }
async function searchHoldTargets(search: string, signal: AbortSignal): Promise<ComboboxOption[]> {
  return (await api<{ options: ComboboxOption[] }>(`/governance/legal-hold-targets?entity_type=${encodeURIComponent(hold.entity_type)}&search=${encodeURIComponent(search)}`, { signal, cacheTtlMs: 0 })).options
}
function chooseHoldTarget(option: ComboboxOption) { hold.entity_id = option.value; hold.entity_label = option.label }
function changeHoldCategory() { hold.entity_id = ''; hold.entity_label = '' }
...
async function execute() {
  if (!selectedPurge.value) return
  await act(`/governance/purge-requests/${selectedPurge.value.id}/execute`, purgeExecution, 'Approved purge executed and its integrity evidence was sealed.')
}
...
<template>
  ...
  <Dialog :open="activeAction === 'hold'" title="Place legal hold" :close-on-backdrop="false" mobile-sheet @close="activeAction = ''"><form @submit.prevent="placeHold"><label>Record category<select v-model="hold.entity_type" data-dialog-initial-focus @change="changeHoldCategory"><option v-for="category in governance?.supported_purge_categories" :key="category" :value="category">{{ category.replaceAll('_', ' ') }}</option></select></label><FloatingCombobox label="Record to protect" :model-value="hold.entity_label" :load-options="searchHoldTargets" placeholder="Search by applicant, reference, purpose, or file" @update:model-value="hold.entity_id = ''; hold.entity_label = $event" @select="chooseHoldTarget" /><label>Reason<textarea v-model="hold.reason" minlength="10" required /></label><button class="button primary full" :disabled="!hold.entity_id">Place hold</button></form></Dialog>
