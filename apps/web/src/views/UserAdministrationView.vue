...
<template>
  ...
  <Dialog :open="activeDialog === 'scopes'" :title="selected ? `Authorisation scopes for ${selected.name}` : 'Authorisation scopes'" description="Replace the complete scope set using reviewed JSON. Non-national scopes require a scope ID." :close-on-backdrop="false" mobile-sheet @close="activeDialog = ''"><FormAlert v-if="error" kind="error" :message="error" /><form v-if="selected && selected.user_type !== 'applicant'" @submit.prevent="saveScopes"><label>Scopes JSON<textarea v-model="scopesText" class="scope-editor" rows="14" spellcheck="false" required data-dialog-initial-focus /><FieldError :error="validationErrors.scopes" /></label><label>Reason for scope change<textarea v-model="reason" minlength="10" rows="3" required /><FieldError :error="validationErrors.reason" /></label><button class="button primary full" :disabled="busy">Replace scopes</button></form></Dialog>
</template>
