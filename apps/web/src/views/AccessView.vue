...
<template>
  <section class="access-layout">
    ...
    <div class="form-panel">
      ...
      <form v-else class="mfa-step" @submit.prevent="changePassword"><p class="eyebrow">Required security step</p><h2>Replace the temporary password</h2><p>Choose a unique password with at least 12 characters, upper- and lower-case letters, and a number. Other application features remain locked until this is complete.</p><label>Current temporary password<input v-model="passwordChange.current_password" type="password" autocomplete="current-password" required /><FieldError :error="fieldErrors.current_password" /></label><label>New password<input v-model="passwordChange.password" type="password" autocomplete="new-password" minlength="12" required /><FieldError :error="fieldErrors.password" /></label><label>Confirm new password<input v-model="passwordChange.password_confirmation" type="password" autocomplete="new-password" minlength="12" required /></label><button class="button primary full" :disabled="busy"><LoadingIndicator v-if="busy" small label="Changing…" /><span v-else>Change password and continue</span></button></form>
    </div>
  </section>
</template>
