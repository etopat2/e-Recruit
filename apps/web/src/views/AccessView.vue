...
onMounted(() => {
  if (session.user?.is_privileged && !session.user.mfa_confirmed) { phase.value = 'enrol'; return }
...
<template>
  <section class="access-layout">
    ...
    <div class="form-panel">
      ...
      <div v-else-if="phase === 'enrol'" class="mfa-step"><p class="eyebrow">Required for staff</p><h2>Protect this account with MFA</h2><p>Enter the current account password to generate an authenticator secret and one-time recovery codes.</p><label>Current account password<input v-model="login.password" type="password" autocomplete="current-password" required /></label><button class="button primary" :disabled="busy || !login.password" @click="enrolMfa">Begin MFA enrolment</button></div>
