...
interface OperationsLookups {
  ...
  panels: LookupRecord[]
  medical_facilities: LookupRecord[]
...
const emptyLookups = (): OperationsLookups => ({
  ...
  medical_facilities: [], medical_schedules: [], selection_outcomes: [], medical_results: [], final_selections: [], training_invites: [],
...
const medicalSchedule = reactive({ recruitment_post_id: '', post_label: '', medical_facility_id: '', facility_label: '', scheduled_date: '', reporting_time: '08:00', capacity: 100 })
...
const medicalFacilityOptions = computed(() => options(lookups.medical_facilities))
...
<template>
  ...
  <Dialog :open="Boolean(activeAction)" :title="activeActionTitle" :close-on-backdrop="false" initial-focus="input, select, textarea" mobile-sheet @close="activeAction = ''">
    ...
    <form v-else-if="activeAction === 'medical-schedule'" @submit.prevent="act('medical-schedule', '/medical/schedules', 'POST', { recruitment_post_id: medicalSchedule.recruitment_post_id, medical_facility_id: medicalSchedule.medical_facility_id, scheduled_date: medicalSchedule.scheduled_date, reporting_time: medicalSchedule.reporting_time, capacity: medicalSchedule.capacity }, 'Medical schedule created inside the authorised post scope.')"><FloatingCombobox label="Recruitment post" :model-value="medicalSchedule.post_label" :options="postOptions" required placeholder="Search active campaign posts" @update:model-value="clearSelection(medicalSchedule, 'recruitment_post_id', 'post_label', $event)" @select="recordSelection(medicalSchedule, 'recruitment_post_id', 'post_label', $event)" /><FloatingCombobox label="Approved medical facility" :model-value="medicalSchedule.facility_label" :options="medicalFacilityOptions" required placeholder="Search facility, town, or prison region" @update:model-value="clearSelection(medicalSchedule, 'medical_facility_id', 'facility_label', $event)" @select="recordSelection(medicalSchedule, 'medical_facility_id', 'facility_label', $event)" /><div class="field-grid"><label>Date<input v-model="medicalSchedule.scheduled_date" type="date" required /></label><label>Reporting time<input v-model="medicalSchedule.reporting_time" type="time" required /></label><label>Capacity<input v-model="medicalSchedule.capacity" type="number" min="1" /></label></div><button class="button primary full" :disabled="busy === 'medical-schedule' || !medicalSchedule.recruitment_post_id || !medicalSchedule.medical_facility_id">Create schedule</button></form>
