...
class StoreApplicationDraftRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'draft_data.education.*.institution_id' => [
                'nullable',
                'ulid',
                Rule::exists('education_institutions', 'id')->where(fn ($query) => $query->where('active', true)),
            ],
            'draft_data.education.*.institution_not_listed' => ['sometimes', 'boolean'],
            'draft_data.education.*.institution' => ['nullable', 'string', 'max:255'],
            'draft_data.education.*.institution_source' => ['nullable', 'string', 'max:30'],
            'draft_data.education.*.institution_registration_status' => ['nullable', 'string', 'max:160'],
